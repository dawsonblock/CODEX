...
## Static Audit Artifacts (Not Dynamically Generated)

The proof package includes three **committed audit reports** that are **NOT dynamically generated by the proof command**. These are historical artifacts from the CODEX-main 34 implementation sprint:

- `memory_schema_reconciliation_report.json` — Committed verification that MemoryProvider schema matches MemoryQuery receiver types
- `governed_memory_routing_report.json` — Committed verification that memory routing decisions are deterministic across refactors
- `event_log_sequence_report.json` — Committed verification that event log ordering is preserved across replay

**Why They Are Static:**
1. These audits verify architectural invariants (schema parity, determinism, sequencing) that do not change on each proof run
2. Regenerating them would require separate audit tooling (not integrated into runtime-cli proof command)
3. They are committed to demonstrate that these audits were performed; re-running them requires explicit manual verification runs

**Verification Status:**
- All three are checked for `pass: true` by `check_proof_manifest_consistency.py`
- Their fields are registered in `proof_manifest.json` and spot-checked for consistency
- They validate correct implementation at commit time; they do not float with proof runs

**Implication for CODEX-main 34:**
- These artifacts provide historical evidence that key architectural invariants were verified
- They should NOT be reinterpreted as "proof command outputs"
- If updating CODEX-main 34's foundation, these audits would need to be manually re-run and re-committed
- They are part of the **bounded verification set** for this release
