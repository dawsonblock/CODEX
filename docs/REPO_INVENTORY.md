...
Current proof values are generated in `artifacts/proof/current/*.json` and validated by
`scripts/check_proof_manifest_consistency.py`. All values below are pinned to the latest proof run.

Key proof reports generated by `cargo run -p runtime-cli -- proof --strict --long-horizon --nl --out ../artifacts/proof/current`:

- `simworld_summary.json` — Main proof pass/fail status
- `replay_report.json` — Replay determinism verification
- `evidence_integrity_report.json` — Proof vault hash chain integrity
- `evidence_claim_link_report.json` — Evidence-claim linkage verification
- `contradiction_integration_report.json` — Contradiction detection and reporting
- `provider_policy_report.json` — Provider execution policy enforcement
- `tool_policy_report.json` — Tool execution policy enforcement
- `reasoning_audit_report.json` — Audit event generation and tracking
- `claim_retrieval_report.json` — Claim retrieval signal verification
- `pressure_replay_report.json` — Operational pressure state and survival
- `nl_benchmark_report.json` — NL diagnostic benchmark (76 scenarios: 15 curated, 59 held-out, 2 adversarial)
- `long_horizon_report.json` — Multi-episode determinism and stability checks

To inspect current values, run: `python scripts/check_proof_manifest_consistency.py`
