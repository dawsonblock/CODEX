...
impl ProofAdmissionTracker {
    ...
    /// Record multiple audits that included governed-memory reason codes.
    pub fn record_audits_with_reason_codes(&mut self, count: usize) {
        self.stats.audits_with_governed_memory_reason_codes += count;
    }

    /// Record multiple retrieval plans generated by governed-memory routing.
    pub fn record_retrieval_plans_generated(&mut self, count: usize) {
        self.stats.retrieval_plans_generated += count;
    }
...
mod tests {
    ...
    #[test]
    fn test_tracker_records_audit_and_retrieval_counts() {
        let mut tracker = ProofAdmissionTracker::new();
        tracker.record_audit_with_reason_codes();
        tracker.record_audits_with_reason_codes(2);
        tracker.record_retrieval_plans_generated(5);

        assert_eq!(tracker.stats().audits_with_governed_memory_reason_codes, 3);
        assert_eq!(tracker.stats().retrieval_plans_generated, 5);
    }
}
